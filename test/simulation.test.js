...
contract('Vault', (accounts) => {
	...
	before('deploy contract', async () => {
		...
		this.Vault = await deployProxy(Vault, [
			...
			ether('100000'), // min     perblockRate = 100000 / 1000  = 100
			ether('1000000'), // max
			500, // 5%
	...
	describe('updateAllocatedFunds()', () => {
		...
		it('should distribute correctly after 1000 blocks', async () => {
			...
			//claim tokens
			await this.Vault.claim(ether('60000'), receiver1, 5, signature, {
				from: user1
			});
			...
			//claim 5k tokens from receiver3 tokens
			await this.Vault.claim(ether('5000'), receiver3, 5, signature, {
				from: user1
			});
		...
		it('should distribute correctly after 1500 blocks', async () => {
			...
			const blocksToIncrease = startBlock.add(new BN('1500'));
			...
			//claim 20k tokens from receiver1 tokens
			await this.Vault.claim(ether('20000'), receiver1, 5, signature, {
				from: user1
			});
			...
			//claim 10k tokens from receiver2 tokens
			await this.Vault.claim(ether('10000'), receiver2, 5, signature, {
				from: user1
			});
		...
		it('should distribute correctly after 2000 blocks', async () => {
			...
			const blocksToIncrease = startBlock.add(new BN('2000'));
			...
			//claim 64k tokens from receiver1 tokens
			await this.Vault.claim(ether('64000'), receiver1, 5, signature, {
				from: user1
			});
			...
			//claim 10.5k tokens from receiver2 tokens
			await this.Vault.claim(ether('10500'), receiver2, 5, signature, {
				from: user1
			});
			...
			//claim 15.5k tokens from receiver3 tokens
			await this.Vault.claim(ether('15500'), receiver3, 5, signature, {
				from: user1
			});
